...
void GameState::start(){ 
	...
	resourceManager_.load("playershovel", "playerspade.png");

	wep = std::unique_ptr<Weapon>(new SpinSlashWeapon(&resourceManager_,NULL));

	tileManager_ = std::unique_ptr<TileManager>(new TileManager(&resourceManager_));
	tileManager_->scheduleEntityAdd(new Player(tileManager_.get(),new sf::Sprite(resourceManager_.get("player")),&resourceManager_), Entity::ABOVE_GROUND);
